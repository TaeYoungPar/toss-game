...
import LastTurnResultCard from "@/components/game/LastTurnResultCard";
...
export default function PlayPage() {
  ...
  const floatBursts = useMemo(() => {
    if (!lastTurnSummary) return [];

    return [
      lastTurnSummary.moneyDelta !== 0
        ? `돈 ${formatSigned(lastTurnSummary.moneyDelta, "원")}`
        : null,
      lastTurnSummary.stressDelta !== 0
        ? `스트레스 ${formatSigned(lastTurnSummary.stressDelta)}`
        : null,
      lastTurnSummary.satisfactionDelta !== 0
        ? `만족도 ${formatSigned(lastTurnSummary.satisfactionDelta)}`
        : null,
      lastTurnSummary.healthDelta !== 0
        ? `체력 ${formatSigned(lastTurnSummary.healthDelta)}`
        : null,
  ...
  const floatBurstKey = useMemo(() => {
    if (!lastTurnSummary) return "";

    return [
      lastTurnSummary.source,
      lastTurnSummary.dayBefore,
      lastTurnSummary.dayAfter,
      lastTurnSummary.moneyDelta,
      lastTurnSummary.stressDelta,
      lastTurnSummary.satisfactionDelta,
      lastTurnSummary.healthDelta,
      goalJustCompleted ? "goal" : "",
      lastComboNotice ?? "",
    ].join(":");
  }, [lastTurnSummary, goalJustCompleted, lastComboNotice]);
  ...
  const actionConfigs = useMemo(
    () => [
      ...
      },
      {
        ...
        title: "소확행 소비",
        description:
          "기분 전환용 소비. 멘탈은 풀리지만 현금이 빠르게 줄어든다.",
      ...
      },
      {
        ...
        title: "무리해서 벌기",
        description:
          "오늘 현금을 크게 당기는 선택. 대신 체력과 멘탈 부담이 크다.",
      ...
      },
      {
        ...
        title: "회복 턴",
        description:
          "오늘은 버는 것보다 회복이 우선. 다음 턴을 위한 정비 시간이다.",
  ...
  return (
    <main className="min-h-screen bg-[#EEF3F8] px-4 py-4 pb-32 sm:px-5 sm:py-6 sm:pb-36">
      <div className="mx-auto max-w-2xl">
        <section
          ref={headerRef}
          className="sticky top-0 z-20 -mx-1 rounded-b-[28px] border-x border-b border-white/70 bg-white/90 px-5 py-4 shadow-[0_14px_36px_rgba(148,163,184,0.14)] backdrop-blur"
        >
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="text-[11px] font-semibold tracking-[0.18em] text-sky-600 uppercase">
                salary survival
              </div>
        ...
        <div className="mt-4 space-y-4">
          <section className="rounded-[32px] bg-[linear-gradient(135deg,#0f172a_0%,#1e293b_55%,#0f172a_100%)] px-5 py-5 text-white shadow-[0_20px_60px_rgba(15,23,42,0.16)]">
            ...
            <div className="mt-4 flex flex-wrap gap-2 text-xs">
              ...
              <span className="rounded-full bg-white/10 px-3 py-1">
                업글 {totalUpgradeCount}개
              </span>
              <span className="rounded-full bg-white/10 px-3 py-1">
                최고점수 {bestRun.bestScore.toLocaleString()}
              </span>
            </div>
          </section>
          ...
          <section
            ref={eventSectionRef}
            className="relative scroll-mt-32 overflow-hidden rounded-[32px] border border-white/70 bg-white/92 p-5 shadow-[0_16px_48px_rgba(148,163,184,0.15)] sm:p-6"
          >
            {currentEvent ? (
              <div>
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  {actionConfigs.map((action) => (
                    <ActionCard
                      ...
                      risk={action.risk}
                      combo={action.combo}
                      selected={activeAction === action.id && !currentEvent}
                      onClick={() => {
                        setActiveAction(action.id);
                        doAction(action.id);
                        requestAnimationFrame(() => setActiveAction(null));
                      }}
                    />
                  ))}
                </div>
          ...
          {lastTurnSummary ? (
            <section
              ...
              className="scroll-mt-32 rounded-[28px] border border-white/70 bg-white/92 p-5 shadow-[0_16px_44px_rgba(148,163,184,0.12)]"
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="text-sm font-semibold text-sky-600">
                  ...
                  <div className="mt-1 text-lg font-bold text-slate-900">
                    {lastTurnSummary.title}
                  </div>
                  <p className="mt-2 text-sm leading-6 text-slate-600">
                    {lastTurnSummary.detail}
                  </p>
                </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                <div className="rounded-[22px] bg-emerald-50/70 px-4 py-3">
                  ...
                  <div
                    className={`mt-1 text-base font-bold ${lastTurnSummary.moneyDelta >= 0 ? "text-emerald-700" : "text-rose-600"}`}
                  >
                ...
                <div className="rounded-[22px] bg-rose-50/70 px-4 py-3">
                  ...
                  <div
                    className={`mt-1 text-base font-bold ${lastTurnSummary.stressDelta <= 0 ? "text-emerald-700" : "text-rose-600"}`}
                  >
                    {formatSigned(lastTurnSummary.stressDelta)}
                  </div>
                </div>
                <div className="rounded-[22px] bg-violet-50/70 px-4 py-3">
                  <div className="text-[11px] font-semibold text-violet-700/70">
                    만족도
                  </div>
                  <div
                    className={`mt-1 text-base font-bold ${lastTurnSummary.satisfactionDelta >= 0 ? "text-violet-700" : "text-slate-700"}`}
                  >
                ...
                <div className="rounded-[22px] bg-sky-50/70 px-4 py-3">
                  ...
                  <div
                    className={`mt-1 text-base font-bold ${lastTurnSummary.healthDelta >= 0 ? "text-sky-700" : "text-rose-600"}`}
                  >
      ...
      {floatBursts.length > 0 ? (
        <div
          key={floatBurstKey}
          className="pointer-events-none fixed inset-x-4 bottom-24 z-40 mx-auto flex max-w-md flex-wrap justify-center gap-2 sm:bottom-28"
        >
      ...
      <div className="fixed inset-x-4 bottom-4 z-40 mx-auto max-w-2xl rounded-[24px] border border-slate-200 bg-white/95 px-3 py-3 shadow-[0_20px_50px_rgba(15,23,42,0.14)] backdrop-blur">
        <div className="grid grid-cols-4 gap-2">
          <button
            ...
            className="rounded-2xl bg-[#F8FAFD] px-3 py-3 text-center text-sm font-semibold text-slate-900"
          >
            상태
          </button>
          <button
              ...
              <div className="mt-5 space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-[24px] bg-[#F8FAFD] p-4">
                    <div className="text-sm text-slate-500">현재 자금</div>
                  ...
                  <div className="rounded-[24px] bg-[#F8FAFD] p-4">
                    <div className="text-sm text-slate-500">현재 빌드</div>
                    <div className="mt-1 text-2xl font-bold text-slate-900">
                      {buildLabel}
                    </div>
                    <div className="mt-1 text-xs text-slate-500">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-[24px] bg-[#F8FAFD] p-4">
                    <div className="text-sm text-slate-500">흐름</div>
                    <div className="mt-1 text-lg font-bold text-slate-900">
                      {streakLabel}
                    </div>
                  </div>
                  <div className="rounded-[24px] bg-[#F8FAFD] p-4">
                    <div className="text-sm text-slate-500">성장</div>
                    <div className="mt-1 text-lg font-bold text-slate-900">
                      업글 {totalUpgradeCount}개
                    </div>
                  </div>
              ...
              <div className="mt-5 space-y-4">
                <div className="rounded-[24px] bg-slate-900 p-5 text-white">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2 text-xs">
                    ...
                    {currentGoal?.rewardText ? (
                      <span className="rounded-full bg-emerald-400/20 px-3 py-1 text-emerald-100">
                        {currentGoal.rewardText}
                      </span>
                    ) : null}
                    {currentGoal?.completed ? (
                      <span className="rounded-full bg-sky-400/20 px-3 py-1 text-sky-100">
                        달성 완료
                      </span>
                    ) : null}
                  </div>
                ...
                <div className="rounded-[24px] bg-[#F8FAFD] p-4">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
                    <div className="rounded-2xl bg-white p-3 text-center shadow-sm">
                      <div className="text-xs text-slate-500">안전 루틴</div>
                      <div className="mt-1 text-xl font-bold text-slate-900">
                        {actionCounts.eatSimple}
                      </div>
                    </div>
                    <div className="rounded-2xl bg-white p-3 text-center shadow-sm">
                      <div className="text-xs text-slate-500">소확행</div>
                      <div className="mt-1 text-xl font-bold text-slate-900">
                        {actionCounts.delivery}
                      </div>
                    </div>
                    <div className="rounded-2xl bg-white p-3 text-center shadow-sm">
                      ...
                      <div className="mt-1 text-xl font-bold text-slate-900">
                        {actionCounts.sideJob}
                      </div>
                    </div>
                    <div className="rounded-2xl bg-white p-3 text-center shadow-sm">
                      <div className="text-xs text-slate-500">회복 턴</div>
                      <div className="mt-1 text-xl font-bold text-slate-900">
                        {actionCounts.rest}
                      </div>
                    </div>
              ...
              <div className="mt-5 max-h-[58vh] space-y-2 overflow-y-auto pr-1">
                {recentLogs.length > 0 ? (
                  recentLogs.map((log, index) => (
                    <div
                      key={`${log.day}-${index}`}
                      className="rounded-[20px] border border-slate-100 bg-[#F8FAFD] px-4 py-3 text-sm leading-6 text-slate-700"
                    >
                      <span className="mr-2 rounded-full bg-white px-2 py-1 text-xs font-bold text-slate-900 shadow-sm">
                        Day {log.day}
                      </span>
                      {log.message}
                    </div>
                  ...
                  <div className="rounded-[24px] bg-[#F8FAFD] px-4 py-5 text-sm text-slate-500">
                    아직 기록이 없다.
                  </div>
                )}
              </div>
